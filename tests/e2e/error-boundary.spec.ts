...
async function loginAsAdmin(page: Page) {
  if (!ADMIN_EMAIL || !ADMIN_PASSWORD) {
    if (process.env.CI) {
      throw new Error(
        'ADMIN_EMAIL e ADMIN_PASSWORD sao obrigatorios no CI para o gate de ErrorBoundary admin.'
      );
    }
...
test.describe('Global Error Boundary', () => {
  test('deve exibir fallback e permitir recuperacao apos unhandled rejection', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /algo deu errado/i })).toBeVisible({
      timeout: 15000,
    });
