...
export const ADD_TO_CART_SELECTOR = 'button:has-text("Adicionar"), button:has-text("GARANTIR COMBO")'
...
export async function login(page: Page) {
    ...
    if (!email || !password) {
        throw new Error('Credenciais de teste (TEST_USER_EMAIL, TEST_USER_PASSWORD) não definidas no .env')
    }
    ...
    if (await loginError.isVisible()) {
        throw new Error('Login falhou: Credenciais inválidas. Verifique se o usuário existe no banco de dados local.')
    }
