  ...
  const handleCancelOrder = async (orderId: string) => {
    ...
    try {
      ...
      if (data && data.success) {
        ...
        addToast(
          `Nao foi possivel cancelar o pedido: ${data?.error || 'erro desconhecido'}.`,
          'error'
        );
      }
    } catch (err) {
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            orders.map((order) => (
              <div
                key={order.id}
                className="bg-zinc-900 border border-white/10 rounded-xl p-6 overflow-hidden hover:border-yellow-400/30 transition-colors"
              >
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4 border-b border-white/5 pb-4">
                  <div>
                    <p className="text-sm text-gray-400 flex items-center gap-2">
                      ...
                      {new Date(order.created_at).toLocaleDateString()} as{' '}
                    ...
                    <div key={item.id} className="flex items-center justify-between text-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-gray-300">
                          ...
                          {item.products?.name || 'Produto indisponivel'}
                        </span>
                      </div>
                ...
                <div className="mt-4 pt-4 border-t border-white/10 flex flex-col md:flex-row justify-between items-center text-xs text-gray-500 gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      {order.customer_address}, {order.customer_city} - {order.customer_zipcode}
                    </span>
                  </div>
