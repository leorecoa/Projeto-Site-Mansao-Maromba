...
export default function ReviewForm({ productId, onSuccess }: Props) {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="bg-zinc-900 p-6 rounded-lg border border-white/10 space-y-4">
            ...
            <div>
                ...
                <textarea
                    {...register('comment', { required: 'Comentario e obrigatorio', minLength: { value: 10, message: 'Minimo de 10 caracteres' } })}
                ...
                />
                {errors.comment && <span className="text-xs text-red-500 mt-1">{errors.comment.message}</span>}
            </div>
