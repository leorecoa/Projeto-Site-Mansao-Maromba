...
export default function ProfileForm() {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="bg-zinc-900 p-4 sm:p-6 rounded-xl border border-white/10 space-y-6">
            <h2 className="text-lg sm:text-xl font-bold text-white mb-4">Meus Dados</h2>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="profile-full-name" className="block text-sm text-gray-400 mb-1">Nome Completo</label>
                    <input
                ...
                <div>
                    <label htmlFor="profile-phone" className="block text-sm text-gray-400 mb-1">Telefone</label>
                    <input
                        id="profile-phone"
                        autoComplete="tel"
                        {...register('phone')}
            ...
            >
                {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Save className="w-4 h-4" /> Salvar Alteracoes</>}
            </button>
        </form>
    );
}
