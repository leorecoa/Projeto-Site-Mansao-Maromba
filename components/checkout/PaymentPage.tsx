...
export default function PaymentPage() {
  ...
  const pixCode =
    '00020126580014BR.GOV.BCB.PIX0136123e4567-e89b-12d3-a456-426614174000520400005303986540510.005802BR5913Mansao Maromba6008Sao Paulo62070503***6304E2CA';
  ...
  useEffect(() => {
    async function fetchOrder() {
      ...
      try {
        ...
        logError('PaymentPage.fetchOrder', error);
        addToast('Nao foi possivel carregar o pedido.', 'error');
      } finally {
        setLoading(false);
      }
    }
  ...
  const copyPix = async () => {
    try {
      ...
      addToast('Codigo PIX copiado!', 'success');
    } catch (error) {
      logError('PaymentPage.copyPix', error);
      addToast('Nao foi possivel copiar o codigo PIX.', 'error');
    }
  };
    ...
    <div className="min-h-screen bg-black text-white pt-24 px-4">
      ...
      <div className="max-w-2xl mx-auto bg-zinc-900 border border-white/10 rounded-2xl p-8 text-center">
        ...
        <p className="text-gray-400 mb-8">
          Pedido #{order.id.slice(0, 8)} � Valor: R$ {order.total_amount.toFixed(2)}
        </p>
