    ...
    return (
      <div className="flex flex-col items-center justify-center p-6 space-y-4 bg-white rounded-lg border border-gray-200 shadow-sm">
        ...
        <div className="w-full">
          <label htmlFor="pix-copy-code" className="text-xs font-semibold text-gray-500 uppercase">
            Codigo Copia e Cola
          </label>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div
          ...
          className={`cursor-pointer relative flex flex-col items-center p-4 border-2 rounded-xl transition-all ${
        ...
        >
          <CreditCard
            className={`w-8 h-8 mb-3 ${selectedMethod === 'credit_card' ? 'text-yellow-600' : 'text-gray-400'}`}
          />
          <span className="font-medium text-sm">Cartao de Credito</span>
          {selectedMethod === 'credit_card' && (
            <CheckCircle className="absolute top-2 right-2 w-4 h-4 text-yellow-500" />
          )}
        </div>
        ...
        >
          <QrCode
            className={`w-8 h-8 mb-3 ${selectedMethod === 'pix' ? 'text-green-600' : 'text-gray-400'}`}
          />
        ...
        >
          <FileText
            className={`w-8 h-8 mb-3 ${selectedMethod === 'boleto' ? 'text-blue-600' : 'text-gray-400'}`}
          />
